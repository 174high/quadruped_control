...
- [Planning](#Planning) </br>
    ...
    - [Foot Trajecotry](#Foot-Trajecotry) </br>
...
The robot is modeled as a single rigid body with ground reaction forces at the feet using the Newton-Euler equations of motion. PD control is used to compose the desired center of mass linear and angular accelerations based on the center of mass pose and velocities. A quadratic program is minimized to find the ground reaction forces that produce the desirec center of mass acclerations. The quadratic program has infinitely many solutions. Therefore, in order for the optimization to converge it is constrained, such that the ground reaction forces must exist within a friction cone. The ground reactions forces are transformed into joint torques using the jacobian inverse. If the the jacobian is near singular the pseoduinverse jacobian is used. However, sometimes the pseoduinverse cannot be solved for within a tolerance so the last resort is to use the jacobian transpose. The balance contoller is only used for feet in the stance phase.
...
To demonstrate the robustness of the balance controller the robot is stood upright with the knees 
...
## Launch 
The Drake install instructions recommend extracting the binaries to `/opt`. Add this Drake binaries to your `PATH` to make runnning the visualization tools easier. 
```
